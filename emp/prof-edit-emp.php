...
<body>
    <?php
        ...
        if(isset($_POST['up'])){
            ...
            $check_pass = "select * from employee where emp_email = '$id' and emp_pass = '$c_p' ";
            $query = $con->query($check_pass);

            if ($query->num_rows>0){
                if ($n_p===$n_cp){
                    $sql = "UPDATE employee SET emp_pass = '$n_p' where emp_email = '$id' ";
                    $isql = $con->query($sql);
            ...
            <div class="col-md-10 right">
                ...
                <form center action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST">
                    ...
                    <div class="btn-group " role="group" aria-label="Basic mixed styles example">
                        <button type="submit" name="up" class="btn btn-success">Change Password</button>
                    </div>
                </form>
                </div>
            </div>
